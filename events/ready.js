...
module.exports = async (bot) => {
    ...
    try {
      ...
      if (eventsToday.length !== 0) {
        ...
        for (const event of eventsToday) {
          ...
          const message = await reqChannel.messages.fetch(event.eventMessageID)
  ...
  async function updateRoleCategoryName(guild) {
    try {
      ...
      await categoryChannel.setName(`🏆Team LSX : ${roleMemberCount} Pilotes🏆`)
    } catch (error) {
      const embedErrorDetectionLog = new Discord.EmbedBuilder()
        .setColor(Config.colors.mainServerColor)
        .setTitle("📌 Erreur Détecté :")
      ...
      await bot.channels.cache
        .get(Config.channels.errorlogChannel)
        .send({ embeds: [embedErrorDetectionLog] })
      await interaction.reply({ embeds: [embedErrorDetected], ephemeral: true })
    }
  }
  ...
  async function fetchRemoteDataFromServer(serverConfig) {
    ...
    try {
      ...
      const settingStream = await downloadToBuffer("/cfg/current/settings.txt")
      ...
      for (const server of servers) {
        ...
        if (server.serverStat === "public") {
          if (server.serverEmbedMessageID === "") {
            ...
            const channel = await bot.channels.cache.get("1132938881411596379")
